...
namespace Arbiter
{
    public partial class EmpleadoVen : System.Web.UI.Page
    {
        ...
        public void CargarTodoelaño()
        {
            ...
            string select = " select DISTINCT  em.nombre, max(co.montocompra)as montomax from Compra co, empleado em where em.id=co.fk_empleado  group by em.nombre order by montomax desc ";
        ...
        {
            ConexionBDAdmin objBD = new ConexionBDAdmin();
            string select = " select DISTINCT  em.nombre, max(co.montocompra)as montomax from Compra co, empleado em where em.id=co.fk_empleado and       co.fechacompra=10-"+mes+"-2015 group by em.nombre order by montomax desc ";
            DataTable dtReporte = objBD.ReporteVentas(select, "Empleado", "Monto BSF");
            GridView1.DataSource = dtReporte;
            GridView1.DataBind();
            GridView1.Visible = true;
