...
namespace Arbiter
{
    public partial class VentasTal : System.Web.UI.Page
    {
        ...
        public void CargarTodoelaño()
        {
            ...
            string select = "select copo.talla, count(copo.fk_producto) 	from Producto po, compraproducto copo	where po.id = copo.fk_producto and		  po.tipo = '" + DDLTipo.SelectedItem.Text + "'  	group by copo.talla	order by count(copo.fk_producto)desc";
            DataTable dtReporte = objBD.ReporteVentas(select, "Talla", "Cantida vendida");
        ...
        {
            ConexionBDAdmin objBD = new ConexionBDAdmin();
            string select = "select copo.talla, count(copo.fk_producto) 	from Producto po, compraproducto copo, compra co	where po.id = copo.fk_producto and		  co.id=copo.fk_compra and		  co.fechacompra = 10-" + mes + "-2015 and		  po.tipo = '" + DDLTipo.SelectedItem.Text + "'  	group by copo.talla	order by count(copo.fk_producto)desc";
            DataTable dtReporte = objBD.ReporteVentas(select, "Talla", "Cantida vendida");
            GridView1.DataSource = dtReporte;
            GridView1.DataBind();
            GridView1.Visible = true;
