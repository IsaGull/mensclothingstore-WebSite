...
namespace Arbiter
{
    public partial class ClientesFre : System.Web.UI.Page
    {
        ...
        public void CargarTodoelaño()
        {
            ...
            string select = "select  cl.nombre, count(co.fk_cliente) from Compra co, cliente cl where cl.id=co.fk_cliente  group by cl.nombre order by count(co.fk_cliente)desc";
            DataTable dtReporte = objBD.ReporteVentas(select, "Cliente", "Cantidad de visitas");
        ...
        {
            ConexionBDAdmin objBD = new ConexionBDAdmin();
            string select = "select  cl.nombre, count(co.fk_cliente) from Compra co, cliente cl where cl.id=co.fk_cliente and       co.fechacompra=10-" + mes + "-2015 group by cl.nombre order by count(co.fk_cliente)desc";          
            DataTable dtReporte = objBD.ReporteVentas(select, "Cliente", "Cantidad de visitas");
            GridView1.DataSource = dtReporte;
            GridView1.DataBind();
            GridView1.Visible = true;
